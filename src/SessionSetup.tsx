...
export default function SessionSetup() {
  ...
  const handleStart = () => {
    ...
    setTimeout(() => {
      setCountdown(0);
      navigate(`/interview?type=${focusType}&difficulty=standard${targetCompany ? `&company=${encodeURIComponent(targetCompany)}` : ""}&role=${encodeURIComponent(targetRole)}&length=${sessionLength}${useResume ? "" : "&useResume=false"}${jobDescription.trim() ? `&jd=${encodeURIComponent(jobDescription.trim().slice(0, 2000))}` : ""}${micStatus === "denied" ? "&nomic=1" : ""}`);
    }, 3000);
  };
