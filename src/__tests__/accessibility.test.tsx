...
describe("Accessibility: Focus Traps", () => {
  ...
  it("keyboard shortcuts modal has role=dialog and aria-modal", async () => {
    const { DashboardProvider } = await import("../DashboardContext");
    const DashboardLayout = (await import("../DashboardLayout")).default;
    await act(async () => {
      render(
    ...
    expect(dialog?.getAttribute("aria-label")).toContain("Keyboard shortcuts");
  });

  it("command palette has role=dialog and aria-modal", async () => {
    const CommandPalette = (await import("../CommandPalette")).default;
    const onStart = vi.fn();
    ...
    const dialog = document.querySelector("[role='dialog'][aria-modal='true']");
    expect(dialog).toBeTruthy();
    expect(dialog?.getAttribute("aria-label")).toContain("Command palette");
  });

  it("command palette traps focus on Tab", async () => {
    const CommandPalette = (await import("../CommandPalette")).default;
    const onStart = vi.fn();
    const onExport = vi.fn();
